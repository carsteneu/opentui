...
| `OPENTUI_GRAPHICS`              | `boolean` | `true`  | Enable Kitty and Sixel graphics detection                       |
...
- `OPENTUI_GRAPHICS=false` disables Kitty and Sixel detection. `OPENTUI_IMAGE_PROTOCOL` overrides automatic protocol selection; explicitly forcing an unsupported protocol can produce incorrect terminal output.
