...
Full-n: `bun scripts/wave3-memory-gate.ts --mutations=10000 --cycles=100 --burst=200` (75.9s).

| Gate | Wert | Limit | OK |
| --- | --- | --- | --- |
| A1 Heapfenster-Median am Workload-Ende | 9,599,590 B | ≤14,020,609 (9,826,305 + max(5%,4MiB)), 312 Fenster | PASS |
| A2 Queue-HWM ≤ 1+1 | 0/0 (via CodeRenderable gecoalest; direkt in C bewiesen) | ≤1/≤1 | PASS |
...
| B2 Lifecycle-Heapfenster nicht wachsend | 6,415,295 B | ≤10,390,087 (warm 6,195,783) | PASS |
...
Keine unhandled Rejections, kein Late Commit, kein Prozessrest (bun-test scheitert sonst; Worker nach destroy terminiert). Rohdaten:
`.yesmem/bench/wave3-memory/2026-08-19T10-16-23-034Z/candidate-raw.json`.
...
## Handover für den seriellen Integrations-Endlauf (§12/§14)

1. Formale n=30 A/B (p99) mit `BUN_PATH=… bun scripts/wave3-memory-ab.ts --runs=30` — daraus den Konfidenz-Intervall-Win ableiten.
